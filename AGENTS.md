...
- `pdflatex` is invoked with `-shell-escape` (required for the running page header).
  Snippet listings are generated first by `make preprocess` (`python3 -m tools.kactl preprocess`).
