...
int main() {
	{
		...
		assert(res.size() == 2);
		assert(dist(res[1]-P(sqrt(2)/2, sqrt(2)/2)) < 1e-8);
	}
	{
		auto res = circleLine(P(0, 0), 1, P(-5,  1), P(5, 1));
		assert(res.size() == 1);
		assert(dist(res[0]-P(0,1)) < 1e-8);
	}
	{
	...
	rep(it,0,100000) {
		...
		if (dist(a - c) < r - 1e-6 || dist(b - c) < r - 1e-6 || dist((a + b) / 2.0 - c) < r - 1e-6) {
			assert(!points.empty());
		}
	}
