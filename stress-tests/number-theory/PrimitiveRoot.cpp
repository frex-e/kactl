...
int main() {
	assert(ord(1, 2) == 1);
	assert(ord(4, 7) == 3); // Wikipedia multiplicative order
	...
	// ACL hardcoded NTT primes
	assert(primitiveRoot(167772161) == 3);
	assert(primitiveRoot(469762049) == 3);
	assert(primitiveRoot(754974721) == 11);
	...
	rep(it,0,15) {
		ull p;
		do { p = rng() % 1000000000ull | 1; } while (p < 3 || !isPrime(p));
		ull g = primitiveRoot(p);
		assert(g >= 1 && g < p);
