...
| `content/graph/LinkCutTree.h` | still unrooted; path lazy via `binop`/`rev`/`applyUpdate`/`mergeUpdate` (default add+sum); point set/get, `findRoot`, `lca`; usage in code comments |
...
- `stress-tests/graph/LinkCutTree.cpp` covers link/cut connectivity plus path sum/add, point set, and rooted LCA
