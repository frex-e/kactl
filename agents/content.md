...
6. Rebuild: `make kactl` or `make web-pdf` for the PDF (both run preprocess). The snippets site picks it up after `npm run index` (or `predev`/`prebuild`), which is the same preprocess step.
