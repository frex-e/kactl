...
\section{Network flow}
	...
	\subsection{Demands and lower bounds}
		...
		cap $\infty$, find a feasible circulation, then
		max-augment $s\to t$ in residual. Or binary search
		on $F$ with lower bound $F$ on $t\to s$ (same as
		\texttt{demand[s]+=F}, \texttt{demand[t]-=F} after
		the rewrite); maximize feasible $F$.
