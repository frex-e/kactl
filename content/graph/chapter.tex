...
\section{Network flow}
	...
	\subsection{Demands and lower bounds}
		...
		Vertex demand $d(v)$ (positive = must receive $d$ more
		inflow than outflow): add source $S$ and sink $T$,
		edge $S\to v$ cap $d(v)$ if $d>0$, $v\to T$ cap
		$-d(v)$ if $d<0$. Feasible iff $\sum d=0$ and max flow
		$S\to T$ equals $\sum_{d>0} d(v)$. A circulation is
		this check after the lower-bound rewrite (then
		$d\equiv 0$ except for the forced $L$ units).

		Max $s$--$t$ flow with lower bounds: after the rewrite,
		binary search on value $F$; for each $F$ set
		\texttt{demand[s]-=F}, \texttt{demand[t]+=F} and check
		feasibility (circulation of value $F$). Equivalently,
		add edge $t\to s$ with lower bound $F$ (cap $\infty$)
		and test circulation; maximize $F$.
